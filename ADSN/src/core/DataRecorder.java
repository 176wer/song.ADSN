...
public class DataRecorder implements ProcessListListener {
	...
	private void addServersListeners() {
		if (serverGroup == null)
			throw new IllegalStateException("No server group defined.");

		MysqlServer[] servers = ServerManager.getInstance().getServersByGroup(serverGroup);
	...
	public synchronized void onProcessListGenerated(ProcessList processlist) {
		...
		if (cells[index] == null) {
			...
			if (cells[index].getSeconds() == seconds) {
				...
				List<ProcessList> list = cells[index].getList();
				storeProcessList(list.get(0).getDate(), list, true); // NOT
																		// OVERRIDE
																		// FILE!
				...
				List<ProcessList> newList = new ArrayList<ProcessList>();
				newList.add(processlist);
				storeProcessList(processlist.getDate(), newList, true); // NOT
																		// OVERRIDE
			}
		}
	}
	...
	private void storeProcessList(Date date, List<ProcessList> processListes, boolean doNotOverrideFile) {
		...
		if (doNotOverrideFile && new File(filename).exists())
			return;
	...
	private String buildProcessListText(ProcessList processlist) {
		...
		sb.append("Server: ").append(processlist.getMysqlServer().getName()).append(LINE_SEPARATOR)
		...
		if (items != null) {
			...
			for (ProcessListItem item : items) {
				...
				if (itemCommand == null || !itemCommand.equals("Sleep")) {
					sb.append(item.getPid()).append("\t\t").append(item.getUser()).append("\t\t").append(item.getHost())
							.append("\t\t").append(item.getDb()).append("\t\t").append(item.getCommand()).append("\t\t")
							.append(item.getTime()).append("\t\t").append(item.getState()).append("\t\t")
							.append(item.getInfo()).append(LINE_SEPARATOR);
				} else
					sleeping++;
			}
			if (sleeping > 0)
				sb.append("Plus ").append(sleeping).append(" sleeping connections.");
		}
		return sb.toString();
	}
		...
		try {
			if (rrd == null) {
				if (new File(filepath).exists())
					rrd = new RrdDb(filepath);
				else
					rrd = createRrd(filepath);
	...
	private long getDirectoryDiskUsage(String directory) {
		...
		for (File file : files) {
			if (file.isDirectory())
				size += getDirectoryDiskUsage(file.getAbsolutePath());
			else
				size += file.length();
		}
		return size;
	...
	public String[] getAvailableChartData() {
		...
		for (File file : files) {
			if (file.getName().endsWith(".rrd"))
				charts.add(file.getName().substring(0, file.getName().length() - 4));
		}
		if (charts.size() > 0)
			return charts.toArray(new String[0]);
		return null;
	}
		...
		try {
			...
			while (entries.hasMoreElements()) {
				...
				if (size > 0) {
					...
					map.put(ze.getName().substring(0, ze.getName().length() - 4),
							sb.substring(0, sb.length() - LINE_SEPARATOR.length()));
				}
			}
			return map;
		} catch (IOException e) {
